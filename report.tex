...
\begin{table}[h]
\centering
\caption{Cross-task performance (seed=1). Walker and cheetah use 1M training frames. Best per column in \textbf{bold}. $^\dagger$Mod~B walker distractor collapsed completely (reward 26.6). $^\ddagger$Cheetah distractor Mod~B and Mod~A+B were still training at submission time.}
...
+ Mod B$^\dagger$  & $852.9$ & $730.6$ & $963.6$ & $26.6$  & $\mathbf{773.2}$ & ---$^\ddagger$ \\
+ Mod A+B      & $749.5$ & $\mathbf{769.8}$ & $907.0$ & $950.8$ & $565.3$ & ---$^\ddagger$ \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Cheetah run (distractor):} The baseline retains reasonable performance ($595.6$, $80.2\%$ of clean), but Mod~A drops substantially ($409.5$, $62.4\%$ of clean), suggesting that task-agnostic augmentation invariance may discard useful spatial information in the richer cheetah observation space under distractors.
