...
\textbf{Comparison with CURL.}
Unlike CURL~\citep{srinivas2020curl}, which is \emph{contrastive} (InfoNCE with in-batch negatives, momentum encoder, projection head, measurable overhead), Mod~A is \emph{non-contrastive}: it matches two views of the same image via MSE, requires no negatives or extra networks, and reuses the augmented views DrQ-v2 already computes---resulting in zero overhead.
...
\textbf{Walker walk:} All methods achieve $>900$ clean. Under distractors, the baseline maintains near-identical performance ($959.3$ vs.\ $958.4$, within noise), suggesting walker is inherently robust to backgrounds. However, \textbf{Mod~B catastrophically fails} ($26.6$), the most severe failure in our experiments, indicating that Q-value pairing breaks down under distractors for locomotion.

\textbf{Cheetah run:} Mod~B excels on clean ($773.2$, $+30.5$ over baseline), suggesting task-aware contrastive learning benefits high-dimensional locomotion. Under distractors, Mod~B ($625.9$) and Mod~A+B ($617.9$) outperform the baseline ($595.6$), but Mod~A drops to $409.5$ ($62.4\%$ retention), indicating that task-agnostic invariance may discard spatial information critical to cheetah.

These single-seed results demonstrate that \textbf{no single auxiliary loss dominates all tasks}. We caution that single-seed results may not be statistically reliable.
