...
Our results suggest that auxiliary losses are task-dependent: the zero-overhead consistency loss is the most practical improvement on cartpole and walker, while task-conditional contrastive learning excels on cheetah but risks catastrophic failure.
\end{abstract}
...
+ Mod A+B      & $749.5$ & $\mathbf{769.8}$ & $907.0$ & $950.8$ & $705.0$ & $617.9$ \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Cheetah run (clean):} Results reveal an interesting task-dependent pattern. Mod~B achieves the highest reward ($773.2$), outperforming the baseline by $30.5$ points, suggesting that task-aware contrastive learning is particularly beneficial for the high-dimensional cheetah locomotion task. Mod~A+B shows a moderate drop ($705.0$), suggesting some interference between the two losses on this task.

\textbf{Cheetah run (distractor):} The baseline retains reasonable performance ($595.6$, $80.2\%$ of clean). Mod~B ($625.9$, $80.9\%$ of its clean) outperforms the baseline under distractors, suggesting that task-aware contrastive learning provides genuine distractor robustness on cheetah. Mod~A+B retains $87.6\%$ ($617.9$ vs.\ clean $705.0$). However, Mod~A drops substantially ($409.5$, $62.4\%$ of clean), indicating that task-agnostic augmentation invariance may discard useful spatial information in the richer cheetah observation space under distractors.

These single-seed cross-task results suggest that \textbf{no single auxiliary loss dominates across all tasks}, highlighting the importance of task-specific evaluation. We caution that single-seed results may not be statistically reliable.
