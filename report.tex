...
This limitation is particularly concerning for real-world deployment, where visual conditions are rarely as controlled as in standard benchmarks.
...
\textbf{Computational cost:} One additional encoder forward pass per update step (for the second augmented view).
...
This creates a controlled but challenging visual distraction setting while maintaining identical physics and reward structure to the clean environment.
...
The diagonal clean-vs-distractor CKA values directly measure representation invariance to distractors:

\begin{center}
\begin{tabular}{lc}
\toprule
\textbf{Method} & \textbf{CKA} (clean $\leftrightarrow$ distractor) \\
\midrule
Baseline & 0.54 \\
Mod A    & 0.63 \\
Mod B    & 0.66 \\
...
The combination (Mod~A+B) produces the most distractor-invariant representations (CKA = 0.78), followed by Mod~B (0.66), Mod~A (0.63), and the baseline (0.54).
...
For practitioners deploying DrQ-v2 in visually complex environments, adding Mod~A with $\alpha=0.1$ is a ``free lunch.''
...
The walker distractor result is particularly instructive: while visual distractors \emph{stabilize} Mod~B on cartpole (all 3 seeds succeed), they \emph{destroy} it on walker, suggesting that the interaction between task complexity, observation dimensionality, and contrastive learning is highly non-trivial.
